...
export default function DashboardTab() {
  ...
  const loadData = async () => {
    setLoading(true);
    const [txns, prods] = await Promise.all([DB.getTransactions(), DB.getProducts()]);
  ...
  return (
    <div className="flex flex-col relative h-full">
      ...
      <div className="bg-white rounded-xl px-4 py-3 flex justify-between items-center shadow-sm border border-gray-100 mb-3">
        ...
        <button onClick={loadData} className="text-xs text-primary font-bold bg-primary-light/50 px-3 py-1 rounded-lg">
          تحديث
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-3 pb-8">
        ...
        <div className="bg-white p-1.5 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-1">
          <button onClick={() => setFilterType('today')} className={`flex-1 min-w-[70px] py-1.5 px-2 rounded-lg text-xs font-bold transition-colors ${filterType === 'today' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>اليوم</button>
          <button onClick={() => setFilterType('week')} className={`flex-1 min-w-[70px] py-1.5 px-2 rounded-lg text-xs font-bold transition-colors ${filterType === 'week' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>الأسبوع</button>
          <button onClick={() => setFilterType('month')} className={`flex-1 min-w-[70px] py-1.5 px-2 rounded-lg text-xs font-bold transition-colors ${filterType === 'month' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>الشهر</button>
          <button onClick={() => setFilterType('custom')} className={`flex-1 min-w-[90px] py-1.5 px-2 rounded-lg text-xs font-bold transition-colors flex justify-center items-center gap-1 ${filterType === 'custom' ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}><Calendar size={14} /> مخصص</button>
        </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="w-6 h-6 rounded-md bg-primary-light text-primary flex items-center justify-center"><WalletCards size={12} /></div>
                </div>
                <div className="text-sm font-black text-gray-900 truncate">{formatCurrencySYP(stats.salesSYP)}</div>
              </div>
              ...
              <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="text-[10px] font-bold text-gray-500">إجمالي المبيعات</div>
                  <div className="w-6 h-6 rounded-md bg-blue-50 text-blue-500 flex items-center justify-center"><DollarSign size={12} /></div>
                </div>
                <div className="text-sm font-black text-gray-900 truncate">{formatCurrencyUSD(stats.salesUSD)}</div>
              </div>
                        ...
                        return (
                          <React.Fragment key={txn.id}>
                            ...
                            >
                              ...
                              <td className="py-3 px-3 text-gray-600 font-medium" dir="ltr">{formatDate(txn.timestamp)}</td>
