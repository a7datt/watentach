...
export default function InventoryTab() {
  ...
  const handleSave = () => {
    ...
    if (modalMode === 'add') {
      ...
      newProducts = newProducts.map(p => {
        if (p.id === formId) {
          return {
            ...p, name: formName, barcode: formBarcode, cost_usd: costStr, price_usd: priceStr, quantity: qtyStr
          };
        }
        return p;
      });
  ...
  const handleDelete = () => {
    ...
    const newProducts = products.filter(p => p.id !== confirmDeleteId);
    DB.saveProducts(newProducts);
    setProducts(newProducts);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setModalMode(null)}>
          <div className="bg-white w-full max-w-[360px] rounded-2xl shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3 border-t border-gray-100 bg-gray-50 flex gap-2">
              ...
              <button onClick={handleSave} className="flex-1 h-10 rounded-lg font-bold bg-primary text-white text-sm flex items-center justify-center gap-1.5"><Save size={16}/> حفظ</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={() => setConfirmDeleteId(null)}>
          <div className="bg-white w-full max-w-[300px] rounded-2xl shadow-xl p-5 text-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleDelete} className="flex-1 h-10 rounded-lg font-bold bg-danger text-white text-sm">تأكيد</button>
            </div>
          </div>
        </div>
      )}
